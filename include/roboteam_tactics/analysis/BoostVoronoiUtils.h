...
public:
  ...
  static void clip(const voronoi_edge<CT> &edge,
      ...
      point_type& point1, point_type& point2) {
    // Don't process curved edges.
    if (edge.is_curved())
      return;

    if (edge.is_finite()) {
      clip(point_type(edge.vertex0()->x(), edge.vertex0()->y()), point_type(edge.vertex1()->x(), edge.vertex1()->y()),
          brect, clipped_edge);
    } else {
      const typename voronoi_edge<CT>::voronoi_cell_type *cell1 = edge.cell();
      const typename voronoi_edge<CT>::voronoi_cell_type *cell2 =
      ...
      */
      if (edge.vertex0()) {
        // Ray edge.
        point_type origin(edge.vertex0()->x(), edge.vertex0()->y());
