...
Vector2 getTargetPos(std::string target, int theirID, bool target_our_team) {
    ...
    if (target == "theirpenaltyline") {
        roboteam_msgs::GeometryFieldSize field = LastWorld::get_field();
        return Vector2(field.right_penalty_line.begin.x, 0);
    }
    if (target == "ourrpenaltyline") {
        roboteam_msgs::GeometryFieldSize field = LastWorld::get_field();
...
// Calculates the distance between the testPosition and a robot
    double calcDistGoalToRobot(int robot, roboteam_msgs::World world) {
        ...
        Vector2 robotPos(9999, 9999);
        try {
            robotPos = Vector2(world.us.at(robot).pos);
        }catch(const std::out_of_range& ex){
            ROS_WARN_STREAM_NAMED("utils.cpp", "Out of range exception in calcDistGoalToRobot! " << ex.what());
        }
