...
#include "roboteam_tactics/conditions/KeeperHasBall.h"
...
RTT_REGISTER_CONDITION(KeeperHasBall);

KeeperHasBall::KeeperHasBall(std::string name, bt::Blackboard::Ptr blackboard) : Condition(name, blackboard) {
    
}

boost::optional<roboteam_msgs::WorldRobot> KeeperHasBall::find_bot_pos(const roboteam_msgs::World& world) const {
...
bt::Node::Status KeeperHasBall::Update() {
    int keeperID = RobotDealer::get_keeper();
    us = GetBool("our_team");
    roboteam_msgs::World world = LastWorld::get();
    auto opt_bot = getWorldBot(keeperID, us);
    if (!opt_bot) {
...
std::vector<roboteam_msgs::World> KeeperHasBall::success_states() const {
...
std::vector<roboteam_msgs::World> KeeperHasBall::fail_states() const {
    roboteam_msgs::World succ1, succ2;
    succ1.us.push_back(roboteam_msgs::WorldRobot());
    succ1.us[0].angle = M_PI_2l; //90 deg
